...
const AUTH_CONFIGURATION_ERROR =
  SECURE_COOKIES && AUTH_SECRET === DEFAULT_AUTH_SECRET
    ? 'Set AUTH_SECRET or SESSION_SECRET in production before enabling admin sign-in.'
    : '';

function authMiddleware(req, res, next) {
  const cookies = parseCookies(req.headers.cookie || '');
  const session = AUTH_CONFIGURATION_ERROR
    ? null
    : verifyToken(cookies[SESSION_COOKIE], 'admin-session');
